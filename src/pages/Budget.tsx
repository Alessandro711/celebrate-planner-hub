      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card className="gradient-romantic border-none">
            ...
            <CardContent>
              {editingTotalBudget ? (
                <Input type="number" value={tempTotalBudget} onChange={e => setTempTotalBudget(Number(e.target.value))} className="text-2xl font-bold h-auto py-1" />
              ) : (
          ...
          <Card>
            ...
            <CardContent>
              <div className={`text-3xl font-bold ${remaining < 0 ? 'text-destructive' : 'text-success'}`}>{formatCurrency(remaining)}</div>
              <p className="text-xs text-muted-foreground mt-1">{remaining < 0 ? 'Acima do orçamento!' : 'Disponível para gastar'}</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Wallet className="h-5 w-5 text-primary" />Categorias de Gastos</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {categories.map(category => {
                ...
                return (
                  <div key={category.id} className="space-y-2 p-4 rounded-lg border border-border hover:bg-muted/30 transition-colors">
                        ...
                        {isEditing ? (
                          <>
                            <div className="flex items-center gap-2">
                              ...
                              <Input type="number" value={editValues.planned} onChange={e => setEditValues({ ...editValues, planned: Number(e.target.value) })} className="w-28 h-8" />
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-xs text-muted-foreground">Gasto:</span>
                              <Input type="number" value={editValues.spent} onChange={e => setEditValues({ ...editValues, spent: Number(e.target.value) })} className="w-28 h-8" />
                            </div>
                            <Button size="sm" onClick={() => handleSave(category.id)}><Save className="h-4 w-4" /></Button>
                          </>
                        ) : (
                          <>
                            <div className="text-right">
                              <p className={`font-medium ${isOverBudget ? 'text-destructive' : ''}`}>{formatCurrency(category.spent)}</p>
                              <p className="text-xs text-muted-foreground">de {formatCurrency(category.planned)}</p>
                            </div>
                            <Button variant="ghost" size="icon" onClick={() => handleEdit(category)}><Edit2 className="h-4 w-4" /></Button>
                          </>
                        )}
                      </div>
                    </div>
                    {!isEditing && (
                      <div className="flex items-center gap-2">
                        <Progress value={Math.min(progress, 100)} className={`h-2 flex-1 ${isOverBudget ? '[&>div]:bg-destructive' : ''}`} />
                        <span className={`text-xs font-medium w-12 text-right ${isOverBudget ? 'text-destructive' : 'text-muted-foreground'}`}>{progress.toFixed(0)}%</span>
                      </div>
                    )}
                  </div>
                );
